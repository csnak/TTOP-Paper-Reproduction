...
# Added second output file to store separate models for separate years
output_folder_2 = './job_output_cons_prior/'
...
og_dir = getwd()
if (CHANGE_DIR) { setwd("..") }
# input_file = "C:/TTO_/data/TTO_dataset_510.csv"  

# Added line for the constant prior dataset
input_file = "C:/TTO_/data/TTO_dataset_510_cons.csv"

D0 <- read_csv(input_file) 
if (CHANGE_DIR) { setwd(og_dir) }
D <- D0 %>% filter(YEAR %in% yr) ### get YRS from config fig
